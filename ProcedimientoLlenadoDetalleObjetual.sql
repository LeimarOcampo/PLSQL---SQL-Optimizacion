create or replace procedure fillDetalleObj(nRows IN number) is
...
begin
    ...
    forall k in 0..(li-1)
        insert into detalle values(codD(k),codProd(k),nroUnits(k),uniValue(k),
            (select ref(f)
             from factura f
             where f.codigof = codFact(k))
            );
    commit;
end;